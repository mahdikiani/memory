...
async def find_entity(tenant_id: str, entity_type: str, name: str) -> Entity | None:
...
async def upsert_entity(
    ...
    existing = await find_entity(tenant_id, entity_type, name)
    ...
    if existing:
        ...
        # Generate record ID
        record_id = f"entity:{tenant_id}:{entity_type}:{name.lower().replace(' ', '_')}"
...
async def link_entity_to_source(entity_id: str, source_id: str) -> None:
    """
